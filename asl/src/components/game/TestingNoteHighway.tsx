...
export function TestingNoteHighway({ notes, elapsed, streak = 0 }: TestingNoteHighwayProps) {

    // Determine glow class based on streak
    const getStreakGlowClass = () => {
        if (streak >= 5) return 'animate-[streak-fire-glow_0.8s_ease-in-out_infinite]';
        if (streak >= 2) return 'animate-[streak-glow-pulse_1s_ease-in-out_infinite]';
        return '';
    };
    ...
    return (
        <div className={`relative w-64 h-[600px] overflow-hidden bg-black/40 border-x-2 border-purple-500/30 rounded-lg mx-8 backdrop-blur-sm transition-all duration-300 ${getStreakGlowClass()}`}>
            {/* Track Gradient */}
